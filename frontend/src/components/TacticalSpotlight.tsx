...
const STYLE_CFG: Record<string, { label: string; icon: string; color: string; bg: string; border: string }> = {
  possession: { label: 'Posse de Bola',  icon: '⚽', color: 'text-blue-400',   bg: 'bg-blue-950/50',   border: 'border-blue-800/60'   },
  counter:    { label: 'Contra-Ataque',  icon: '⚡', color: 'text-yellow-400', bg: 'bg-yellow-950/50', border: 'border-yellow-800/60' },
  pressing:   { label: 'Pressão Alta',   icon: '🔥', color: 'text-orange-400', bg: 'bg-orange-950/50', border: 'border-orange-800/60' },
  defensive:  { label: 'Bloco Def.',     icon: '🛡️', color: 'text-slate-400',  bg: 'bg-slate-800/50',  border: 'border-slate-700/60'  },
};
  ...
  return (
    <div className={`flex-1 rounded-2xl border p-5 flex flex-col gap-3 ${cfg.bg} ${cfg.border}`}>
      <div className={`flex items-start gap-2 ${isAway ? 'flex-row-reverse text-right' : ''}`}>
        <div className="text-2xl leading-none shrink-0">{cfg.icon}</div>
        <div>
...
export function TacticalSpotlight() {
  ...
  // Determina o dominante da posse sem exibir percentual
  const dominantPossession = (home.gameDominanceProb ?? 50) >= (away.gameDominanceProb ?? 50)
    ? { team: match.team1, style: home.dominanceStyle }
    : { team: match.team2, style: away.dominanceStyle };
  const dominantCfg = STYLE_CFG[dominantPossession.style] ?? STYLE_CFG['pressing'];
    ...
    <section className="px-4 py-10" id="spotlight">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 md:p-8 space-y-6">
          ...
          {/* Provável dominante da posse — sem percentual */}
          <div className={`flex items-center gap-3 rounded-2xl border px-5 py-3 ${dominantCfg.bg} ${dominantCfg.border}`}>
            <span className="text-xl">{dominantCfg.icon}</span>
            <div>
              <p className="text-[10px] uppercase text-slate-500 font-bold tracking-widest">Provável dominante da posse</p>
              <p className={`text-base font-black ${dominantCfg.color}`}>{dominantPossession.team}</p>
            </div>
          </div>
          ...
          {match.aiAnalysis && (
            <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
              <p className="text-[10px] text-slate-600 uppercase tracking-widest font-bold mb-1.5">🧠 Resumo Tático</p>
